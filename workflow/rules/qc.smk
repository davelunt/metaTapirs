...
rule fastp_trim_reads:
    input:
        config['input_data'] + "/{LIBRARIES}/{SAMPLES}.fastq.gz",
    output:
        trimmed = "results/02_trimmed/{LIBRARIES}/{SAMPLES}.trimmed.fastq.gz",
        failed = "results/02_trimmed/{LIBRARIES}/{SAMPLES}.trimmed.failed.fastq.gz",
...
rule seqkit_fq2fa:
    input:
        fq = "results/02_trimmed/{LIBRARIES}/{SAMPLES}.trimmed.fastq.gz",
    output:
